...
export type ExtendedColumnDef<T> = ColumnDef<T> & {
  ...
  tooltip?: string;
  /** Congela a coluna na borda. "left" = esquerda, "right" = direita */
  freeze?: "left" | "right";
};
...
const DataGrid = <T extends object>({
  ...
  // Estado de colunas congeladas (inicializa pelo columnDef, editável pelo cog)
  ...
  // Larguras reais medidas do DOM para calcular offsets sticky
  ...
  useEffect(() => {
    if (!autoPageSizeOnDesktop || !up("nt") || !rootRef.current) {
      setCalculatedPageSize(undefined);
      setGridHeight(undefined);
      return;
    }

    const rootEl = rootRef.current;
    const toolbarEl = rootEl.querySelector(
      `.${styles.toolbar}`,
    ) as HTMLElement | null;
    const footerEl = rootEl.querySelector(
      `.${styles.footer}`,
    ) as HTMLElement | null;
    const theadEl = tableRef.current?.querySelector(
      "thead",
    ) as HTMLElement | null;
  ...
  // Mede larguras reais das colunas — só re-executa quando colunas/freeze mudam,
  // NÃO em cada render (evita loop render → medição → setState → render)
  useEffect(() => {
    if (!tableRef.current) return;
    // Só precisa medir se há colunas congeladas
  ...
  // ─── Colunas internas (memoizadas para evitar recriação em cada render) ────
  ...
  const rowNumberColumn = useMemo<ExtendedColumnDef<T>>(
    () => ({
      ...
      header: "#",
      cell: ({ row }) => (
        <span className={styles.rowNumber}>{row.index + 1}</span>
      ),
      colSize: "fit",
      width: 52,
      textAlign: "center",
  ...
  const allColumns = useMemo<ExtendedColumnDef<T>[]>(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [showRowSelection, showRowNumbers, columns],
  );
          ...
          ((info: any) => {
            ...
            if (colDef.badge) {
              ...
              if (b)
                return (
                  <div
                    className={`${styles.cellWithBadge} justify-content-center`}
                  >
  ...
  const startRow = (currentPage - 1) * pageSizeValue + 1;
  const endRow = Math.min(currentPage * pageSizeValue, totalRows);

  // ─── Calcula offsets sticky para freeze ─────────────────────────────────────
  const freezeInfo = useMemo(() => {
    const visibleIds = table.getVisibleLeafColumns().map((c) => c.id);

    // left: acumula da esquerda
    ...
    // Último left e primeiro right (recebem a sombra divisória)
      ...
      visCols.map((col) => {
        const cell = row
          ...
          .find(
            (c) =>
              c.column.id ===
              String((col as any).id ?? (col as any).accessorKey ?? ""),
          );
        return formatCellValue(cell?.getValue(), col);
      }),
    );
  ...
  // Paginação (memoizada — só recalcula quando página/total muda)
  ...
  const [showRefreshLine, setShowRefreshLine] = useState(false);
  ...
  useEffect(() => {
    ...
    setShowRefreshLine(true);
    if (refreshTimeoutRef.current) {
      window.clearTimeout(refreshTimeoutRef.current);
    }
    refreshTimeoutRef.current = window.setTimeout(() => {
      setShowRefreshLine(false);
  ...
  return (
    <div
      ...
      className={`${styles.root} ${className} mt-4 ${compact ? styles.compact : ""}`}
    >
      <div className={styles.refreshBar}>
        <div
          className={`${styles.refreshFill} ${showRefreshLine ? styles.refreshFillActive : ""}`}
        />
      </div>
      {/* TOOLBAR — só aparece se tiver título, export ou actions */}
      {(title ||
        subtitle ||
        showExport ||
        actions ||
        ...
        <div className={styles.footerInfo}>
          {!loading && totalRows > 0 ? (
            <span>
              ...
              {data.length !== totalRows && ` (filtrado de ${data.length})`}
            </span>
          ) : !loading && data.length === 0 ? null : null}
        </div>
